        ...
        <div className='flex flex-col h-full'>
            ...
            <ul className='flex flex-col w-full my-5 gap-[2px]'>
                ...
                {Object.keys(users || {}).length > 0 && filteredChats?.map((chat) => {
                    ...
                    return (
                        <li key={chat[0]} className={`h-[90px] flex items-center gap-4 rounded-3xl hover:bg-c1 p-4 cursor-pointer ${selectedChat?.uid === user?.uid ? 'bg-c1' : ''}`} onClick={() => handleSelect(user, chat[0])}>
